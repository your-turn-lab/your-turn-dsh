...
test('one start saves preferences, submits the task, and starts the visible path',()=>{
  const s=send(onboard(),'START_DEMO');
...
test('pausing blocks queued ticks and resuming continues the same node',()=>{
  let s=tick(send(onboard(),'START_DEMO'));
...
test('opening history pauses automatic work and closing resumes the current substep',()=>{
  let s=tick(send(onboard(),'START_DEMO'));
  s=send(s,'OPEN_VIEW',{view:'history'});
  assert.equal(s.view,'history');
  assert.deepEqual(tick(s),s);
  s=send(s,'CLOSE_VIEW');s=tick(s);
  assert.equal(s.autoIndex,1);
});
