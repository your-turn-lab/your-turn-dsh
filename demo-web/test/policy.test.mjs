...
test('changing preferences during a judgment uses the saved budget at future candidates',()=>{
  ...
  s=send(s,'SAVE_PROFILE',{answers:[1,1,1,0]});
...
test('critical override is retained from the plugin, even when ordinary recall budget is exhausted',()=>{
  const preferenceProfile=preferenceFromAnswers([1,1,1,0]);
  const s={...createState(),preferenceProfile,recallState:{recallCount:1}};
