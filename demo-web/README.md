...
- `src/vendor/question-card.css` is extracted from `@deepseek-ai/dsh-client-ui-user-questions@0.1.2-rc.1`. Question card markup and the surrounding chat shell are independent implementations using the original card class names, hierarchy and copy conventions. The DSH shell is not a full runtime replica. Minimal demo labels, progression buttons and long-form presentation sizing are added outside the plugin.
...
The main prompt and Recall Policy numbers are provisional, configured in `scenario.mjs`. Input remarks are recorded in outputs. Alternative supported choices change the agenda and interaction script. Arbitrary free text does not trigger model reasoning: the technology-department revision is a configured scenario, and other text is retained as requirements rather than represented as a newly inferred task.
...
1. Answer the four original onboarding questions, then edit retained judgments/delegated work. Save.
2. Confirm **long task + balanced mode**. Other task controls can be previewed but this representative run is explicitly fixed to that pair. Preferences are saved/displayed; they do not silently change this fixed scenario's recalls.
3. Submit Dawn's training request. The original floating task path opens.
4. Advance Auto: material checks, organization and terminology checks are shown as completed fixture work.
5. Your Turn ①: choose **个人办公 → 团队协作 → 知识复用** and confirm the note about the second half.
6. Step through **candidate → evaluation → Auto**. The relationship node stays automated, with no question card and no extra recall count.
7. Your Turn ②: choose low-pressure pain-point voting and confirm the natural-expression note.
8. Inspect/download the 60-minute preparation plan. Accept the result.
9. Open the client change. In the **real path rail**, click **3. 贯穿案例** to pin the drawer. Hover or Tab-focus **确定授课对象与案例**, edit **修改这一步的做法**, then use **保存并从这里重做** (the circular arrow). Paste or enter the technology department requirement shown in the main panel. The first two nodes stay unchanged; click through four dependent nodes. Compare V1 and V2, then accept V2.
10. Switch between first-use and continued-use illustrative weights, or move sliders. Future node routing updates. This is explicitly not longitudinal evidence or the final production algorithm.

The chosen substep editor is the actual plugin editor, including its existing hover/focus behavior. Other nodes remain inspectable; unsupported rerun branches return an explicit scenario-boundary message. Use Escape/close to dismiss a drawer. The original rail can be dragged/resized and collapsed. Header **重新开始** resets the run after confirmation.

## Local verification — 2026-09-12

- Demo unit/isolation tests: **10 passed**.
- Browser tests: **2 passed**, including the full scenario and 1280×720 / 390×844 overflow checks. The full scenario asserts rail synchronization within 500 ms, no page exceptions, and no external requests.
