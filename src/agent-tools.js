...
function acceptedRecallDecision(recallDecision, recallState = {}) {
  const recallCountBefore = recallState.recallCount || 0;
  return {
    ...recallDecision,
    budget: {
      ...recallDecision.budget,
      recallCountBefore,
      recallCount: recallCountBefore + 1,
    },
  };
}
...
export function registerAgentTools(ctx, sessions) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Call request_human_decision for meaningful user checkpoints, aiming for up to three distinct Your Turn moments in a task when there are real direction, ownership, or learning decisions. Provide node-specific tags: preference_dependent, downstream_impact, irreversible, core_judgment, learning_value, ownership_value. Do not ask for minor wording, formatting, naming, or low-impact choices. The same tool call waits for the answer when policy allows the recall.',
