...
const STATUS_LABEL = { pending: '未开始', in_progress: '进行中', waiting_for_user: '已暂停 · 等你参与', awaiting_feedback: '进行中 · AI 正在反馈', feedback_ready: '已暂停 · 等你确认', completed: '已完成' };
...
function substepVisualStatus(node, substep) {
  const pausedSubstep = node.substeps.find((item) => item.status === 'in_progress') ?? node.substeps.find((item) => item.status !== 'completed');
  if (PAUSED_STATUSES.has(node.status) && substep.id === pausedSubstep?.id) return 'paused';
  return substep.status;
}

function taskLifecycle(state) {
  ...
  if (state.pendingDecision || state.nodes.some((node) => PAUSED_STATUSES.has(node.status))) return { key: 'attention', label: '需要你回来' };
  if (state.nodes.every((node) => node.status === 'completed')) {
...
const styles = `
.hil-root{--hil-stage:#f4f4f5;--hil-shell:#f8f8f9;--hil-surface:#fff;--hil-muted:#f3f3f4;--hil-border:#e4e4e7;--hil-ink:#18181b;--hil-copy:#52525b;--hil-caption:#a1a1aa;--hil-success:#10b981;--hil-success-soft:#d1fae5;--hil-active:#f59e0b;--hil-active-soft:#fef3c7;--hil-paused:#ef4444;--hil-paused-soft:#fee2e2;position:fixed;inset:0;z-index:80;pointer-events:none;font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--hil-ink);color-scheme:light}
...
.hil-resize-handle{position:absolute;left:30px;right:30px;height:8px;z-index:4;cursor:ns-resize;touch-action:none}.hil-resize-handle.top{top:0}.hil-resize-handle.bottom{bottom:0}.hil-resize-handle:after{content:'';position:absolute;left:50%;width:32px;height:2px;border-radius:999px;background:#d4d4d8;transform:translateX(-50%)}.hil-resize-handle.top:after{top:3px}.hil-resize-handle.bottom:after{bottom:3px}.hil-resizing{transition:none!important}
@keyframes hil-active-breathe{0%,100%{box-shadow:inset 0 0 0 3px var(--hil-active),0 0 0 0 rgba(245,158,11,.08);opacity:.72}50%{box-shadow:inset 0 0 0 3px var(--hil-active),0 0 0 4px rgba(245,158,11,.18);opacity:1}}@keyframes hil-pause-pulse{0%,100%{box-shadow:0 0 0 0 rgba(239,68,68,.1);opacity:.78}50%{box-shadow:0 0 0 4px rgba(239,68,68,.16);opacity:1}}@keyframes hil-inline-in{from{opacity:0;transform:translateY(-3px)}to{opacity:1;transform:translateY(0)}}@keyframes hil-drawer-in{from{opacity:0;transform:translateX(7px) scale(.99)}to{opacity:1;transform:translateX(0) scale(1)}}@keyframes hil-rail-in{from{opacity:0;transform:translateX(8px)}to{opacity:1;transform:translateX(0)}}
...
function Outcome({ state, busy, onAccept }) {
  return <div className="hil-summary"><h2 className="hil-h2">本次任务的决策影响</h2><p className="hil-copy">只记录当前任务中真实发生的决定与结果变化。</p><div className="hil-label">你的决定</div>{state.outcome.decisions.map((item) => <div className="hil-summary-item" key={item.id}>{item.detail}</div>)}<div className="hil-label">结果因此发生的变化</div>{state.outcome.effects.map((item, index) => <div className="hil-summary-item" key={`${item.decisionId}-${index}`}><strong>{item.before}</strong><br/>→ {item.after}</div>)}{state.finalAcceptedAt ? <div className="hil-summary-item"><strong>最终成果已验收</strong></div> : <button className="hil-btn primary" disabled={busy} onClick={onAccept}>确认验收最终成果</button>}</div>;
}

function substepResultLabel(node, substep, visualStatus) {
  if (visualStatus === 'paused') return '已暂停，等待你介入';
  if (substep.result) {
    const text = String(substep.result).replace(/\s+/gu, ' ').trim();
    if (text.length <= 40) return text;
    const firstClause = text.split(/[。；]/u).find((item) => item.trim().length >= 10)?.trim();
    const summary = firstClause && firstClause.length <= 40 ? firstClause : text.slice(0, 39).trimEnd();
    return `${summary}…`;
  }
...
function HumanLoopOverlay({ call }) {
  ...
  const lifecycle = taskLifecycle(state);
  return <div className="hil-root">
    ...
    {open && <>
      <section ref={railRef} style={railStyle(railPosition, railHeight)} className={`hil-rail ${dragging === 'rail' ? 'hil-dragging' : ''} ${resizing ? 'hil-resizing' : ''}`} aria-label="人机决策路径" onClick={(event) => { if (event.target === event.currentTarget) { setPinnedNodeId(null); setSummaryOpen(false); } }}>
        ...
        <footer className="hil-rail-footer"><div className={`hil-attention-state ${lifecycle.key}`}><span className="hil-state-dot"/><span>{lifecycle.label}</span></div>{state.outcome && <button className="hil-summary-link" onClick={() => { setDrawerTop(76); setPinnedNodeId(null); setSummaryOpen(true); }}>查看决策效果 →</button>}</footer>
      ...
      {drawerOpen && <aside ref={drawerRef} className="hil-drawer" style={drawerStyle(railPosition, drawerTop, viewport, drawerHeight)} aria-label="节点详情" onMouseEnter={keepPreview} onMouseLeave={() => { if (hovered) leavePreview(); }}>
        ...
        <main className="hil-detail">
          {!hovered && summaryOpen ? <Outcome state={state} busy={busy} onAccept={() => dispatch({ type: 'ACCEPT_FINAL_RESULT' })}/> : !detailNode ? <div className="hil-card"><strong>等待 Agent 发布路径</strong></div> : <>
            ...
            <details className="hil-more"><summary>更多信息</summary>{!hovered && !suggestion && detailNode.status !== 'completed' && <><div className="hil-label">谁来完成</div><div className="hil-buttons">{Object.entries(MODE_LABEL).map(([mode,label]) => <button key={mode} className={`hil-btn ${detailNode.mode === mode ? 'primary' : ''}`} onClick={() => dispatch({ type: 'CHANGE_MODE', nodeId: detailNode.id, mode })}>{label}</button>)}</div></>}<div className="hil-label">选择这种方式的原因</div><p className="hil-copy">{detailNode.rationale}</p>{!!detailNode.evidence.length && <><div className="hil-label">材料与证据</div><ul className="hil-list">{detailNode.evidence.map((item) => <li key={item}>{item}</li>)}</ul></>}{!hovered && detailNode.status !== 'completed' && <><div className="hil-label">调整整个大步骤</div><textarea className="hil-textarea" value={draft} onChange={(event) => setDraft(event.target.value)}/><button className="hil-btn" onClick={() => dispatch({ type: 'EDIT_INSTRUCTION', nodeId: detailNode.id, instruction: draft })}>保存整体要求</button></>}</details>
          </>}
