...
function TaskProfileControls({ profile = {}, busy, onChange }) {
  ...
  return <div className="hil-card hil-profile"><div className="hil-label">任务规模</div><div className="hil-segment">{Object.entries(TASK_SIZE_LABEL).map(([value, label]) => <button key={value} className={`hil-segment-btn ${taskSize === value ? 'active' : ''}`} disabled={busy} onClick={() => onChange({ taskSize: value })}>{label}</button>)}</div><div className="hil-label">参与目标</div><div className="hil-segment">{Object.entries(PARTICIPATION_GOAL_LABEL).map(([value, label]) => <button key={value} className={`hil-segment-btn ${participationGoal === value ? 'active' : ''}`} disabled={busy} onClick={() => onChange({ participationGoal: value })}>{label}</button>)}</div></div>;
}
...
function RecallPolicyDebug({ state }) {
  ...
  return <div className="hil-card hil-debug"><strong>Recall Policy</strong><div className="hil-debug-grid"><span>Recall Value</span><b>{formatScore(latest?.recallValue)}</b><span>Threshold</span><b>{formatScore(latest?.threshold)}</b><span>Decision</span><b>{latest?.action ?? '—'}</b><span>Auto Risk</span><b>{formatScore(latest?.autoRisk)}</b><span>Human Value</span><b>{formatScore(latest?.humanValue)}</b><span>Task Size</span><b>{TASK_SIZE_LABEL[budget?.taskSize ?? profile.taskSize] ?? '中等任务'}</b><span>Goal</span><b>{PARTICIPATION_GOAL_LABEL[budget?.participationGoal ?? profile.participationGoal] ?? '平衡模式'}</b><span>Recall Count</span><b>{budget ? `${budget.recallCount} / ${budget.maxRecall}` : `${state.recallState?.recallCount ?? 0} / —`}</b></div></div>;
}
