...
function RecallPolicyDebug({ state, node }) {
  ...
  return <div className="hil-card hil-debug"><strong>Recall Policy</strong><div className="hil-debug-grid"><span>Recall Value</span><b>{formatScore(decision.recallValue)}</b><span>Threshold</span><b>{formatScore(decision.threshold ?? budget?.threshold)}</b><span>Decision</span><b>{decision.action ?? '—'}</b><span>Auto Risk</span><b>{formatScore(decision.autoRisk)}</b><span>Human Value</span><b>{formatScore(decision.humanValue)}</b><span>Task Size</span><b>{TASK_SIZE_LABEL[budget?.taskSize ?? profile.taskSize] ?? '中等任务'}</b><span>Goal</span><b>{PARTICIPATION_GOAL_LABEL[budget?.participationGoal ?? profile.participationGoal] ?? '平衡模式'}</b><span>Recall Count</span><b>{budget ? `${budget.recallCount} / ${budget.maxRecall}` : `${state.recallState?.recallCount ?? 0} / —`}</b></div></div>;
}
